  ...
  <div class="max-w-xl mx-auto py-4">
    <section class="bg-zinc-900 border border-zinc-800 rounded-xl p-4 sm:p-6 shadow-xl">
      ...
      <form @submit.prevent="handlePublish" class="space-y-4">
        <div>
          <label class="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1">Ваш никнейм</label>
          <input 
            v-model="authorName"
            type="text"
            required
            placeholder="Например, cyber_punk" 
            class="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 text-zinc-100 transition-colors"
          />
        </div>
